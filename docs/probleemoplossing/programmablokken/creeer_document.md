...
Indien de kolom _Naam sjabloon in Xential_ (dvnaaminexternsjablprog) gevuld is in de sjabloondefinitie (beheertabel tbdocumenten), dan zal OpenWave Xential aanroepen om een document te genereren. Zie [Xential](/docs/probleemoplossing/programmablokken/xential.md).
...
Indien echter bovenstaande het geval is, maar de de [satellite](/docs/instellen_inrichten/satellite_filesysteem.md) staat aan, dan werkt dit alleen indien de kolom _Tekst_ van de instelling _Sectie: Documenten en Item: Documentroot_ gelijk is aan dezelfde instelling van de configuratiefile van de geïnstalleerde satellite.
