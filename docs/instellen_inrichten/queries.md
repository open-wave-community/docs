...
OpenWave heeft zelf een aantal functies op de database gedefinieerd - zoals fn_ddmaandjjjj() - die gebruikt kunnen worden in allerlei queries. Zie: [OpenWave database functies](/docs/applicatiebeheer/instellen_inrichten/openwave_database-functies).
...
De aanroep van het SQL-statement vindt plaats via de tegeldefinitie van de tegel waarop het dynamische opschrift moet verschijnen door het vullen van de kolom _Tegelopschrift dynamisch met API gettilecontent_ (zie [Portal tegel](/docs/applicatiebeheer/instellen_inrichten/portaldefinitie/portal_tegel)). Een voorbeeld van een dynamisch tegelopschrift is de waarde `_getTileContent(omgeving_status,{id})`. De codering _omgeving_status_ verwijst naar een uniek codering in de tabel tbqueries. Het SQL-statement dat aldaar staat zal worden gebruikt om het dynamische deel van het tegelopschrift te genereren.
...
Zie [Scherminformatie voor detailschermen](/docs/applicatiebeheer/instellen_inrichten/schermdefinitie/scherminformatie_voor_detailschermen).
...
Zie de voorbeelden hieronder bij [action](/docs/applicatiebeheer/instellen_inrichten/queries#action).
...
Het resultaat van de query moet gevuld zijn met standaard html colorname ([https://www.w3schools.com/colors/colors_names.asp](https://www.w3schools.com/colors/colors_names.asp)).
...
Zie ook kopje **Invoegen tekstblokken op basis van een query-aanroep naar tbqueries** bij [Documentsjablonen](/docs/applicatiebeheer/instellen_inrichten/documentsjablonen).
...
Zie voor invoegen plaatjes op basis van query onder Kopje: _Invoegen plaatjes op basis van een query-aanroep naar tbqueries die verwijst naar tbimages_ bij [Documentsjablonen](/docs/applicatiebeheer/instellen_inrichten/documentsjablonen).
...
Zie het kopje _formquery en childquery-verwijzingen naar tbqueries_ bij [Documentsjablonen en Sjabloongroepen](/docs/applicatiebeheer/instellen_inrichten/documentsjablonen)
